...
class Locker {
  ...
  ghost predicate Valid()
    reads this
  {
    (occupied  ==> parcelId > 0 && ownerId > 0) &&
    (!occupied ==> parcelId == -1 && ownerId == -1)
  }
  ...
  method PlaceParcel(id: int, owner: int, parcelSize: Size)
    requires Valid()
    requires !occupied
    requires id > 0
    requires owner > 0
  ...
  method RemoveParcel(userId: int, isCourier: bool)
    requires Valid()
    requires occupied
    requires userId == ownerId || isCourier
    modifies this
    ensures !occupied
    ensures parcelId == -1
    ensures ownerId == -1
